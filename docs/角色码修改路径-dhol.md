...
这份文档描述的是 `dhol` 项目里“手机角色码房间（`mobile-panel`）”所采用的角色码解析协议，以及当前实现边界。
...
`dhol` 手机角色码房间当前默认支持上游截至 2026-05-31 的 `dhc3_` / 版本 `3` 角色码，同时兼容同布局的 `dhc2_` / 版本 `2` 角色码。
...
1. `dhol` 手机房间默认使用 `dhc3_`，并兼容同布局的新版 `dhc2_`
2. 静态角色信息来自角色码反解
3. 动态资源状态在房间内独立维护
4. 字典必须与上游快照严格一致
5. 出错时宁可明确报错，也不要静默容错
