import type { ImportedCharacterData, ResourceTrackerSheet } from '@dhgc/shared'
...
export function extractCharacterDataFromHtml(html: string): ImportedCharacterData {
  ...
  if (start < 0) {
    throw new Error('未找到 window.characterData，当前 HTML 不是受支持的 MyDHcharsheet 导出格式。')
  }

  const objectStart = html.indexOf('{', start)
  if (objectStart < 0) {
    throw new Error('找到了 characterData 标记，但没有找到对象起始位置。')
  }
  ...
  try {
    ...
    try {
      // Fallback for object literals that still use valid JavaScript but not strict JSON.
      return Function(`"use strict"; return (${objectText});`)() as ImportedCharacterData
    } catch (error) {
      throw new Error(error instanceof Error ? `角色卡数据解析失败：${error.message}` : '角色卡数据解析失败。')
    }
  }
}

export function buildSheetFromImportedCharacterData(value: unknown, fileName: string): ResourceTrackerSheet {
...
export function cloneSheet(sheet: ResourceTrackerSheet): ResourceTrackerSheet {
...
function findObjectLiteralEnd(source: string, objectStart: number) {
  let depth = 0
  let inString = false
  let quote = ''
  let escaped = false
  ...
  throw new Error('characterData 对象没有正常结束。')
}
